...
function switchLeaveTab(tab) {
    const tabs = document.querySelectorAll('#leave-screen .tab-btn');
...
// Replace goToProfile logic
js = js.replace(/function goToProfile\(\) \{[\s\S]*?document\.getElementById\('profile-screen'\)\.classList\.add\('active'\);[\s\S]*?\}/, `function goToProfile() { handleNavigation('profile-screen'); }`);

// Replace goToHome logic
js = js.replace(/function goToHome\(\) \{[\s\S]*?document\.getElementById\('home-screen'\)\.classList\.add\('active'\);[\s\S]*?\}/, `function goToHome() { handleNavigation('home-screen'); }`);

// Replace goToAttendance logic
js = js.replace(/function goToAttendance\(\) \{[\s\S]*?document\.getElementById\('attendance-screen'\)\.classList\.add\('active'\);[\s\S]*?\}/, `function goToAttendance() { handleNavigation('attendance-screen'); }`);

if (!js.includes('function closeAllScreens')) {
    js += '\n' + newFunctions;
    fs.writeFileSync('Mobile/js/app.js', js);
}
console.log('Updated app.js navigation logic.');
