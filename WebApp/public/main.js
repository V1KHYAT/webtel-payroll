    ...
    function switchTab(tabId, btn) {
      ...
      const parent = btn && typeof btn.closest === 'function'
        ? (btn.closest('.wizard-layout') || btn.closest('.card') || document)
        : document;
      ...
      // Keep sidebar buttons and mobile dropdown in sync for wizard-style forms.
      parent.querySelectorAll('.wizard-nav button').forEach(button => {
        const clickHandler = button.getAttribute('onclick') || '';
        if (clickHandler.includes(`'${tabId}'`) || clickHandler.includes(`\"${tabId}\"`)) {
          button.classList.add('active');
        }
      });
      parent.querySelectorAll('.wizard-sidebar-mobile select').forEach(select => {
        if (select.value !== tabId) select.value = tabId;
      });
    }
    ...
    // Ensure floating bulk action bar reflects any pre-checked rows on initial page load.
    document.addEventListener('DOMContentLoaded', checkBulkActions);
    ...
    function getSavedThemeColor() {
      // Support environments where localStorage may be partitioned/unavailable.
      try {
        return localStorage.getItem('hrp-theme-color');
      } catch (_e) {
        return null;
      }
    }

    function persistThemeColor(color) {
      try {
        localStorage.setItem('hrp-theme-color', color);
      } catch (_e) {
        // Ignore storage failures; URL fallback still carries theme across pages.
      }
    }

    function updateThemeLinks(color) {
      document.querySelectorAll('a[href]').forEach(link => {
        ...
        try {
          ...
          // Ignore malformed URLs and keep original links.
        }
      });
    }
    ...
    function getThemeFromQueryParam() {
      ...
      return value && themeColorMap[value] ? value : null;
    }
    ...
    function setThemeColor(color) {
      ...
      updateThemeLinks(color);
      // Update active swatch indicator (only on pages that have these buttons)
...
// Initialize theme on page load (runs on all pages automatically)
(function() {
  const themeFromUrl = getThemeFromQueryParam();
  const savedTheme = getSavedThemeColor();
  const activeTheme = themeFromUrl || (savedTheme && themeColorMap[savedTheme] ? savedTheme : null);
