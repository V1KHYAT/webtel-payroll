...
// Also replace expand chevrons since they might have class="expand-chevron"
html = html.replace(/<i data-lucide="([^"]+)" class="expand-chevron"><\/i>/g, (match, iconName) => {
    const solarIcon = iconMap[iconName] || `${iconName}-linear`;
    return `<iconify-icon icon="solar:${solarIcon}" class="expand-chevron"></iconify-icon>`;
});
